...
public class ReservationController : ControllerBase
{
    ...
    public async Task<ActionResult<Item>> Reserve(Item reservation, [FromServices] DaprClient daprClient)
    {
        ...
        state.Value.Changes ??= new List<ItemReservation>(); // Fix for CS8602: Ensure Changes is not null before accessing it
    ...
    [HttpPost("balance/{state}")]
    public async Task<ActionResult<Item>> Set(Item reservation, [FromServices] DaprClient daprClient)
    {
        var state = await daprClient.GetStateEntryAsync<ItemState>(StoreName, reservation.SKU);
        Console.WriteLine($"Current balance for {state.Value.SKU} is {state.Value.BalanceQuantity}");
